...
python run.py \
  ...
  trainer.max_epochs=null ++trainer.max_steps=10000 \
  ++diffusion.noise_distribution="uniform" ++diffusion.rho_train=7 \
  diffusion.loss_function=wmse diffusion.sigma_max_inf=800 \
  ++diffusion.sigma_max=800 ++diffusion.sigma_min_train=0.002 \
  ...
  module.optimizer.lr=3e-4 module.optimizer.muon.lr=3e-3 module.optimizer.weight_decay=0.05 module.optimizer.muon.wd=0 \
  ...
  ++datamodule.loss_latitude_weighting=True \
  name_suffix="EDM-12h-W2r-MC-3l_33lr_4gp" suffix="UNIF_base" \
  "$@"